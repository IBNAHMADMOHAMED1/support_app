...
<form method="post" action="{{route('ticket.update', $ticket->id)}}">
  ...
  <div class="editor mx-auto w-10/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl">
    ...
    <input type="hidden" name="serviceid" value="{{$ticket->services->id }}">
    <input type="hidden" name="userid" value="{{Auth::user()->id}}">
    <input name="title" class="title bg-gray-100 border border-gray-300 p-2 mb-4 outline-none" spellcheck="false" placeholder="Title" type="text" value="{{ $ticket->title }}">
    <textarea name="description" id="editor" class="description bg-gray-100 sec p-3 h-60 border border-gray-300 outline-none" spellcheck="false" placeholder="Describe everythnig about this Content her">
        {{ $ticket->description }}
    </textarea>
